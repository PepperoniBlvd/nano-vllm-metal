...
def test_paged_cache_matches_full_cache():
    page_size = 4
...
def test_paged_pool_reuses_freed_pages():
...
def test_model_kv_caches_share_layer_pools():
    mlx_model = _fake_qwen3_mlx_model()
    paged_model = Qwen3PagedModel(mlx_model, page_size=4)
    first_request_cache = paged_model.create_kv_cache()
    second_request_cache = paged_model.create_kv_cache()

    assert len(first_request_cache) == paged_model.num_hidden_layers
    # Each layer has its OWN pool so per-layer KV scatter-writes stay disjoint and
    # MLX can donate them in place (a single shared pool serializes all layers'
    # writes into one chain MLX must copy, ~130x slower). Requests share the
    # per-layer pools, which is what lets prefix caching reuse pages.
    for layer in range(paged_model.num_hidden_layers):
        assert first_request_cache[layer].pool is paged_model.page_pools[layer]
        assert second_request_cache[layer].pool is paged_model.page_pools[layer]

    # Different layers use different pools; page metadata is independent.
...
def test_model_layer_caches_keep_independent_page_metadata():
    mlx_model = _fake_qwen3_mlx_model()
    paged_model = Qwen3PagedModel(mlx_model, page_size=4)
    cache = paged_model.create_kv_cache()
    inputs = mx.array([[1, 5, 7, 3, 9]], dtype=mx.int32)

    paged_model(inputs, 0, cache)

    assert cache[0].page_ids == [0, 1]
    assert cache[0].page_lens == [4, 1]
    # Each layer allocates from its OWN pool, so every layer independently gets
    # the same page ids (each pool numbers from 0) and the same page lengths, and
    # reads back through its own pool.
    for layer in range(paged_model.num_hidden_layers):
        ...
        for page_id in cache[layer].page_ids:
            key_page, value_page = paged_model.page_pools[layer].read_page(page_id)
            assert key_page.shape[2] == paged_model.page_size
            assert value_page.shape[2] == paged_model.page_size


def test_incremental_decode_matches_dense():
    mlx_model = _fake_qwen3_mlx_model()
    dense_model = Qwen3DenseModel(mlx_model)
    paged_model = Qwen3PagedModel(mlx_model, page_size=4)
    inputs = mx.array([[1, 5, 7, 3, 9, 11]], dtype=mx.int32)
    dense_cache = dense_model.create_kv_cache()
    paged_cache = paged_model.create_kv_cache()

    for offset in range(inputs.shape[1]):
        token = inputs[:, offset : offset + 1]
        dense_out = dense_model(token, offset, dense_cache)
        paged_out = paged_model(token, offset, paged_cache)
        dense_out = dense_out - mx.logsumexp(dense_out, keepdims=True)
        paged_out = paged_out - mx.logsumexp(paged_out, keepdims=True)
        assert_allclose(
            paged_out, dense_out, precision=mx.bfloat16, rtol=1e-3, atol=1e-3
        )
