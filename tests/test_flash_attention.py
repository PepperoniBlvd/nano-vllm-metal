...
def test_flash_attention_cpu(mask_mode: str):
    attention_helper(mx.cpu, 18, 6, 7, 5, 3, 10, mask_mode)


@pytest.mark.parametrize("mask_mode", ["no_mask", "mask", "causal"])
def test_flash_attention_cpu_large(mask_mode: str):
    attention_helper(mx.cpu, 28, 4, 16, 128, 16, 3, mask_mode)


def test_flash_attention_cpu_causal_mask_faster_than_all_zero_mask():
...
@pytest.mark.parametrize("mask_mode", ["no_mask", "mask"])
def test_flash_attention_gpu_extra_small(mask_mode: str):
    attention_helper(mx.gpu, 1, 1, 5, 7, 4, 1, mask_mode)


@pytest.mark.parametrize("mask_mode", ["no_mask", "mask", "causal"])
def test_flash_attention_gpu_small(mask_mode: str):
    attention_helper(mx.gpu, 6, 3, 2, 5, 3, 1, mask_mode)


@pytest.mark.parametrize("mask_mode", ["no_mask", "mask"])
def test_flash_attention_gpu(mask_mode: str):
    attention_helper(mx.gpu, 18, 6, 7, 5, 3, 10, mask_mode)


@pytest.mark.parametrize("mask_mode", ["no_mask", "mask", "causal"])
def test_flash_attention_gpu_large(mask_mode: str):
    attention_helper(mx.gpu, 28, 4, 16, 128, 16, 3, mask_mode)


def test_flash_attention_gpu_causal_mask_faster_than_all_zero_mask():
    assert_causal_mask_faster_than_all_zero_mask(
        stream=mx.gpu,
        batch=2,
        h_q=8,
