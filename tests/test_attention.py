...
def test_softmax(stream: mx.Stream, precision: mx.Dtype):
...
def test_simple_attention_scale_mask(
    stream: mx.Stream, precision: mx.Dtype, batch_dimension: int
):
    """
...
@pytest.mark.parametrize("stream", AVAILABLE_STREAMS, ids=AVAILABLE_STREAMS_IDS)
@pytest.mark.parametrize("precision", PRECISIONS, ids=PRECISION_IDS)
def test_linear(stream: mx.Stream, precision: mx.Dtype):
    with mx.stream(stream):
        BATCH_SIZE = 10
        DIM_Y = 10
        DIM_X = 12
...
@pytest.mark.parametrize("stream", AVAILABLE_STREAMS, ids=AVAILABLE_STREAMS_IDS)
@pytest.mark.parametrize("precision", PRECISIONS, ids=PRECISION_IDS)
def test_simple_multi_head_attention(stream: mx.Stream, precision: mx.Dtype):
    """
