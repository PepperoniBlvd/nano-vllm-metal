...
from .linear import QuantizedWeights
from .activation import silu
from .linear import quantized_linear
...
class Moe:
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        expanded_x = mx.broadcast_to(
            mx.expand_dims(x, -2), (B, L, self.num_experts_per_tok, D)
        ).reshape(-1, D)
        flat_expert_ids = expert_ids.reshape(-1)
        ...
        expert_output = grouped_expert_linear(
            silu(gate) * up, self.w_down, flat_expert_ids
