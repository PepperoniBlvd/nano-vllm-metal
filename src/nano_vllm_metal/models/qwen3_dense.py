...
from ..layers.attention import scaled_dot_product_attention_grouped, flash_attention
...
class Qwen3MultiHeadAttention:
    ...
    def __call__(
        ...
        B, L, _ = x.shape
        query = quantized_linear(x, self.wq).reshape(
            B, L, self.num_heads, self.head_dim
        )
        key = quantized_linear(x, self.wk).reshape(
            B, L, self.num_kv_heads, self.head_dim
        )
        value = quantized_linear(x, self.wv).reshape(
            B, L, self.num_kv_heads, self.head_dim
        )
        ...
        query = self.rope(query, offset=offset_slice).transpose(0, 2, 1, 3)
        key = self.rope(key, offset=offset_slice).transpose(0, 2, 1, 3)
        value = value.transpose(0, 2, 1, 3)
        key, value, _, mask = cache.update_and_fetch(
            key, value, mask_length=L, mask=mask
        )
        if self.use_flash_attention:
            ...
            # Fused SDPA kernel: handles grouped-query attention and accumulates
            # in fp32 internally, so we skip the explicit float32 upcast of
            # Q/K/V (which for decode re-materialized the whole float32 K/V cache
            # every step). Inputs stay in the model dtype.
...
class Qwen3TransformerBlock:
    ...
    def __call__(
        ...
        hidden = x + self.self_attn(self.input_layernorm(x), offset, cache, mask)
...
class Qwen3DenseModel:
    ...
    def create_kv_cache(self) -> list[TinyKvCache]:
        from ..cache.kv_cache import TinyKvFullCache
