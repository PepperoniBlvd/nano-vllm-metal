...
class PagedAttentionInt8Decode : public mx::Primitive {
public:
    explicit PagedAttentionInt8Decode(mx::Stream stream, const float scale, const bool is_causal,
                                      const int num_kv_heads, const int num_heads)
        : mx::Primitive(stream), scale_(scale), is_causal_(is_causal), num_kv_heads_(num_kv_heads), num_heads_(num_heads) {};
    ...
    std::pair<std::vector<mx::array>, std::vector<int>> vmap(const std::vector<mx::array> &inputs,
                                                             const std::vector<int> &axes) override {
        throw std::runtime_error("PagedAttentionInt8Decode has no vmap implementation.");
    }

    const char *name() const override { return "PagedAttentionInt8Decode"; }
