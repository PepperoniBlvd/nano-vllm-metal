...
void QuantizedMatmul::eval_gpu(const std::vector<mx::array> &inputs, std::vector<mx::array> &outputs) {
    ...
    const bool is_f16 = out.dtype() == mx::float16;

    // SIMD-group GEMV kernel: one SIMD group (32 lanes) cooperatively reduces
    // the contraction dimension for a single output element, with coalesced
    // reads of the quantized weights and a simd_sum reduction. It handles both
    // decode (one A row) and prefill (many A rows, one output element each) far
    // faster than the generic one-thread-per-output kernel. It splits N across
    // 32 lanes at pack (8-value) granularity, so it needs N to be a multiple of
    // 32 * 8 = 256; other shapes fall back to the generic kernel.
    const bool use_gemv = (N % 256 == 0);

    const char *kernel_name;
    if (use_gemv) {
        kernel_name = is_f16 ? "quantized_matmul_gemv_w4_g128_f16" : "quantized_matmul_gemv_w4_g128_bf16";
    } else {
        kernel_name = is_f16 ? "quantized_matmul_w4a16_g128_f16" : "quantized_matmul_w4a16_g128_bf16";
    }
    ...
    if (use_gemv) {
        // One SIMD group (32 lanes) computes one output column. Pack several
        // SIMD groups per threadgroup; grid.y iterates the rows of a.
        const int simdgroups_per_tg = 8;
        const int threads_x = simdgroups_per_tg * 32;
        MTL::Size num_threadgroups =
            MTL::Size((K + simdgroups_per_tg - 1) / simdgroups_per_tg, M, 1);
        MTL::Size num_threads_per_group = MTL::Size(threads_x, 1, 1);
