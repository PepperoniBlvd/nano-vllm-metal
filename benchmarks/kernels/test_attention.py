...
def test_attention(benchmark):
    with mx.stream(mx.gpu):
        q, k, v, res = get_test_attention_data()
        result = benchmark(
            lambda: nvm.scaled_dot_product_attention_grouped(
                q, k, v, scale=1.0
            )
        )
        assert_allclose(result, res, precision=mx.float32, rtol=1e-2)
...
def test_flash_attention(benchmark):
    with mx.stream(mx.gpu):
        q, k, v, res = get_test_attention_data()
        result = benchmark(lambda: nvm.flash_attention(q, k, v, scale=1.0))
        assert_allclose(result, res, precision=mx.float32, rtol=1e-2)
